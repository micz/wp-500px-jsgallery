...
<table class="form-table">
    <tr valign="top"><th scope="row"><?esc_html_e('500px Username','wp-500px-jsgallery');?></th>
        ...
        <br/><?if($this->options[self::_500px_user]==''){echo '<span style="color:red;font-weight:bold;">'.esc_html__('You must specify a 500px username here or using the "user500px" shortcode param directly on the page!','wp-500px-jsgallery').'<span>';
        }else{
   ...
   <tr valign="top"><th scope="row"><?esc_html_e('Number of thumbnails per page','wp-500px-jsgallery');?></th>
        <td><input type="text" name="wp5jsgal_options[<?=self::_page_thumbs?>]" value="<?php echo $this->options[self::_page_thumbs]; ?>"/></td>
    </tr>
   <tr valign="top"><th scope="row"><?esc_html_e('Thumbnails dimensions','wp-500px-jsgallery');?></th>
        <td><?esc_html_e('Height','wp-500px-jsgallery');?> <input type="text" name="wp5jsgal_options[<?=self::_thumb_h?>]" value="<?php echo $this->options[self::_thumb_h]; ?>"/> <?esc_html_e('px','wp-500px-jsgallery');?><br/>
        <?esc_html_e('Width','wp-500px-jsgallery');?> <input type="text" name="wp5jsgal_options[<?=self::_thumb_w?>]" value="<?php echo $this->options[self::_thumb_w]; ?>"/> <?esc_html_e('px','wp-500px-jsgallery');?><br/>
        <?esc_html_e('The dimensions could be set with the custom CSS, instead of with these options.','wp-500px-jsgallery');?><br/>
        <?esc_html_e('You can also set only one dimension, the other will be proportional.','wp-500px-jsgallery');?></td>
    </tr>
   <tr valign="top"><th scope="row"><?esc_html_e('Image dimensions','wp-500px-jsgallery');?></th>
        <td><?esc_html_e('Max Height','wp-500px-jsgallery');?> <input type="text" name="wp5jsgal_options[<?=self::_image_h?>]" value="<?php echo $this->options[self::_image_h]; ?>"/> <?esc_html_e('px','wp-500px-jsgallery');?><br/>
        <?esc_html_e('Max Width','wp-500px-jsgallery');?> <input type="text" name="wp5jsgal_options[<?=self::_image_w?>]" value="<?php echo $this->options[self::_image_w]; ?>"/> <?esc_html_e('px','wp-500px-jsgallery');?><br/>
        <?esc_html_e('The dimensions could be set with the custom CSS, instead of with these options.','wp-500px-jsgallery');?><br/>
        <?esc_html_e('You can also set only one dimension, the other will be proportional.','wp-500px-jsgallery');?></td>
    </tr>
   <tr valign="top"><th scope="row"><?esc_html_e('Gallery page','wp-500px-jsgallery');?></th>
        <td><input type="text" name="wp5jsgal_options[<?=self::_pages?>]" value="<?php echo $this->options[self::_pages]; ?>"/>
        <br/><?esc_html_e('To optimize your website loading times, you could write here the pages id or permalink on which you have activated the 500px gallery with the shortcode.','wp-500px-jsgallery');?><br/>
        <?esc_html_e('The ids or permalinks must be comma separated and can be mixed.','wp-500px-jsgallery');?><br/>
        <?esc_html_e('All the styles and scripts needed by this plugin will be loaded only on those pages.','wp-500px-jsgallery');?></td>
    </tr>
   <tr valign="top"><th scope="row"><?esc_html_e('Use CSS version 1','wp-500px-jsgallery');?></th>
        <td><input type="checkbox" name="wp5jsgal_options[<?=self::_force_css_v1?>]" value="1"<?php if($this->options[self::_force_css_v1]==1){echo ' checked="checked"';} ?>"/> <?esc_html_e('Check this option if you want to use the old CSS version 1.','wp-500px-jsgallery');?><br/>
        <?esc_html_e('From version 2.0 the CSS is responsive and the thumbnails are displayed under the image.','wp-500px-jsgallery');?><br/>
        <a href="<?=self::url_force_css_v1;?>" target="_blank"><?esc_html_e('More info on the old CSS version 1','wp-500px-jsgallery');?></a></td>
    </tr>
   <tr valign="top"><th scope="row"><?esc_html_e('Exclusive custom CSS','wp-500px-jsgallery');?></th>
        <td><input type="checkbox" name="wp5jsgal_options[<?=self::_only_custom_css?>]" value="1"<?php if($this->options[self::_only_custom_css]==1){echo ' checked="checked"';} ?>"/> <?esc_html_e('Check this option if you want to load only your custom CSS and not the default one before your one.','wp-500px-jsgallery');?><br/>
        <a href="<?=self::url_custom_css_info;?>" target="_blank"><?esc_html_e('More info on custom CSS','wp-500px-jsgallery');?></a></td>
    </tr>
</table>
